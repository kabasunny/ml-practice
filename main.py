...
def fetch_and_prepare_data(sector_number, trade_start_date, before_period_days, end_date):
    ...
    return all_features_df, all_features_all_df, symbol_data_dict
...
def optimize_parameters_for_symbols(symbol_signals, symbol_data_dict):    
    ...
    for symbol, signals in symbol_signals.items():
        ...
        for signal_date in signals:
            ...
            if not any(param["stop_loss_percentage"] == best_result["stop_loss_percentage"] and param["trailing_stop_trigger"] == best_result["trailing_stop_trigger"] and param["trailing_stop_update"] == best_result["trailing_stop_update"] for param in optimal_params):
                optimal_params.append(best_result)
            else:
                rejected_params.append(best_result)
            if not any(param["stop_loss_percentage"] == worst_result["stop_loss_percentage"] and param["trailing_stop_trigger"] == worst_result["trailing_stop_trigger"] and param["trailing_stop_update"] == worst_result["trailing_stop_update"] for param in least_optimal_params):
...
def main():
    ...
    all_features_df, all_features_all_df, symbol_data_dict = fetch_and_prepare_data(sector_number, trade_start_date, before_period_days, end_date)
    ...
    print(f"データ取得、学習データ、特徴量、ラベルの生成 処理時間: {end_time_features - start_time_features:.2f}秒")
    ...
    symbol_signals = model_predict(gbm, model_predict_features_df, all_features_all_df, symbol_data_dict)

# --------------------------将来、別のプロジェクトにて、以下をGoに移管したい

...
# フーリエ変換を用いたサイクル解析関数
def fft_analysis(detrended_prices):
    n = len(detrended_prices)  # 終値データの長さを取得

    # FFT（高速フーリエ変換）
    fft_result = np.fft.fft(detrended_prices)  # FFTを実行
    ...
    fft_amplitude = fft_amplitude[non_zero_idx]  # 振幅スペクトルを対応する部分に絞り込み
    ...
    if len(fft_amplitude) > 1:
        ...
        fft_amplitude = np.delete(fft_amplitude, max_idx)  # 最大値を振幅スペクトルから除去
        fft_period = np.delete(fft_period, max_idx)  # 信頼性が低いと考え、最大周期を周期リストから除去
        print(f"除外した最大周期の値: {excluded_value}")  # 除外した値をアナウンス

    
    dominant_periods = fft_period[np.argsort(-fft_amplitude)[:5]]  # 上位5つの支配的な周期を取得
